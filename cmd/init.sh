...
sudo apt-get update
sudo apt-get -y install apt-fast && sudo apt-fast install zsh fbterm emacs24 mongodb redis-server redis-tools libxss1 && sudo apt-fast upgrade && sudo apt-fast dist-upgrade
